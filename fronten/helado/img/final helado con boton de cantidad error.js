...
const itemMenucarrito =[]
console.log(itemMenucarrito)
...
elementoPadre.addEventListener('click', e => {
   ...
   if(e.target.classList.contains('pcompra_embutido'))
      ...
      /* Agrego elemento al array itemMenucarrito  */
      const agregarAlCarritoItem = productList.find((product) => product.id == productCarrito)
            
            itemMenucarrito.push(agregarAlCarritoItem);
         ...
         if (existente){
            const prod = menucarrito.map (product => {
               if (product.id == productCarrito){
                  ...
                  console.log(product.price += product.priceBace)
               }
            })
...
const actualizarCarritoCaca = () => {
   ...
   sumatoriaDeCarrito.innerText = "$" + " " + itemMenucarrito.reduce((acc , product) => acc + product.priceBace, 0)
}
      ...
      if(e.target.classList.contains('boton-crus'))
         {
            ...
            const eliminarAlCarrito = itemMenucarrito.find((product) => product.id == productCarrito2)
            const indice = menucarrito.indexOf(eliminarAlCarrito)
            itemMenucarrito.splice(indice, 1)
